...
module.exports = {
  ...
  change: async (req, res) => {
    try {
      const { daysToExpires, membershipId, freeMembership } =
        await getUserMembershipData(req.session.user.id);
      ...
      const sameDaysMemberships = memberships.filter((membership) => {
        if (freeMembership) {
          return membership.days === days;
        } else {
          return membership.days === days && membership.order > order;
        }
      });
      const otherDaysMemberships = memberships.filter((membership) => {
        if (freeMembership) {
          return membership.days !== days;
        } else {
          return membership.days !== days && membership.order > order;
      ...
      const membershipsFirstGroup = freeMembership
        ? sameDaysMemberships
        : sameDaysMemberships.map((membership) => {
            if (membership.days === days) {
              const currentPricePerDay = Number(price) / days;
              const membershipPricePerDay =
                Number(membership.price) / membership.days;
